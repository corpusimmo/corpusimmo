...
import { EstimatorBackdrop } from "./estimator-backdrop";
...
const STEP_META: { title: string; subtitle: string }[] = [
  ...
  },
  {
    ...
    subtitle:
      "Un champ facultatif peut rester vide : mieux vaut vide qu'approximatif.",
  },
  {
...
export function EstimatorWizard({ onResult }: EstimatorWizardProps) {
  ...
  useEffect(() => {
    ...
    setRestored(
      !outcome.conflict && Boolean(stored) && (stored?.step ?? 0) > 0,
    );
    setHydrated(true);
  }, [searchParams]);
  ...
  return (
    <div ref={topRef} className="scroll-mt-24">
      {/* Le fond suit la typologie choisie. Rendu ici et non par la page :
          c'est le wizard qui détient l'état, et le faire remonter par un
          contexte pour une image serait payer cher un décor. */}
      <EstimatorBackdrop type={state.type} usage={state.usage} />

      {/* Deux colonnes à partir de 1024 px : le parcours à gauche, la fiche du
          bien à droite, collante pour rester visible pendant qu'on répond. En
          dessous, la fiche passe SOUS le formulaire — elle récapitule, elle
          n'introduit pas, et la mettre en tête retarderait la première
          question. */}
        ...
        <div className="min-w-0">
          ...
          <div className="flex flex-col gap-4">
            <div className="hidden md:block">
              <Stepper
                steps={steps.map((index) => WIZARD_STEPS[index] ?? "")}
                current={position}
              />
            </div>
          ...
          {state.addressLocked && state.address ? (
            <div className="mt-5 flex flex-col gap-2 rounded-md border border-border bg-surface-2 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
              <p className="flex min-w-0 items-start gap-2 text-sm text-ink">
                <MapPin
                  aria-hidden="true"
                  className="mt-0.5 size-4 shrink-0 text-accent-rule"
                />
                  ...
                  <div className="animate-fade-in flex flex-col gap-3 rounded-md border border-danger bg-danger-soft px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex gap-3">
                      ...
                      <div className="text-sm text-danger-soft-fg">
                        ...
                        <p>
                          Nous n&apos;avons pas réussi à joindre le moteur
                          d&apos;estimation. Vos réponses conservées, vous
                          pouvez réessayer immédiatement.
                        </p>
                      </div>
                    </div>
                    <Button
                ...
                >
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <p className="text-sm text-ink-muted">
                      Recherche des comparables, pondération, calcul de la
                      fourchette.
                    </p>
                  </div>
                </div>
  ...
  return (
    <div className="rounded-lg border border-border bg-surface p-5 shadow-sm sm:p-8">
      ...
      <p className="mt-2 text-sm leading-relaxed text-ink-muted">
        Vous aviez commencé une estimation, et vous venez d&apos;en lancer une
        autre pour une adresse différente. Nous ne remplaçons rien sans vous le
        demander.
      </p>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        <Choice
          ...
          detail={`Saisie déjà commencée${
            draft.step > 0
              ? `, arrêtée à l'étape « ${WIZARD_STEPS[draft.step] ?? ""} »`
              : ""
          }.`}
          action={
            <Button
