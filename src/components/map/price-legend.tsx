  ...
  if (compact) {
    return (
      <div
        className={shell}
        role="img"
        aria-label={`Légende : prix au m² en cinq classes, de moins de ${formatNumber(first)} à plus de ${formatNumber(last)} euros`}
      >
  ...
  return (
    <div className={shell}>
      <p className="text-[11px] font-medium text-ink">
        Prix au m², sur les ventes affichées
      </p>
      ...
      <p className="mt-1.5 text-[10px] leading-snug text-ink-subtle">
        Quintiles des {formatNumber(scale.sample)} ventes au m² connu. Les
        bornes suivent la vue.
      </p>
    </div>
  );
}
